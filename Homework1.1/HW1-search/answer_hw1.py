...
def general_serach(problem, structure, heuristic=lambda *x: 0):
    ...
    if isinstance(structure, util.PriorityQueue):
        structure.push(Item(start_pos), 0)
    else:
        structure.push(Item(start_pos))
    while not structure.isEmpty():
        item = structure.pop()
        if item.pos not in visited:
            visited.add(item.pos)
            if problem.isGoalState(item.pos):
                return item.path
            for next_pos, next_action, next_cost in problem.getSuccessors(item.pos):
                new_path = item.path + [next_action]
                new_item = Item(next_pos, new_path, next_cost)
                if isinstance(structure, util.PriorityQueue):
                    new_item.cost += item.cost
                    structure.update(new_item, new_item.cost + heuristic(next_pos, problem))
                else:
